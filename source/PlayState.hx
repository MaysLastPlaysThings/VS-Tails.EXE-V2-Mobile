...
class PlayState extends MusicBeatState
{
	...
	public var eggMap:Map<String, Character> = new Map();
	...
	public var eggMap:Map<String, Character> = new Map<String, Character>();
	...
	public var EGG_X:Float = 0;
	public var EGG_Y:Float = 0;
	...
	public var eggGroup:FlxSpriteGroup;
	...
	var staticazzo0:FlxSprite;
	var staticazzo1:FlxSprite;
	var staticazzo2:FlxSprite;
	var vg:FlxSprite;
	private var animSky:FlxSprite;
	private var animTerrain:FlxSprite;
	private var animStuff:FlxSprite;
	private var tailsPxl:FlxSprite;
	...
	override public function create()
	{
		...
		if (stageData.egg != null)
			EGG_X = stageData.egg[0];
		if (stageData.egg != null)
			EGG_Y = stageData.egg[1];
		...
		eggGroup = new FlxSpriteGroup(EGG_X, EGG_Y);
		gfGroup = new FlxSpriteGroup(GF_X, GF_Y);

		if (!SongUnlock.getUnlock(SONG.song))
			SongUnlock.unlockSong(SONG.song);

		gradient = new FlxSprite().loadGraphic(Paths.image('stages/burningGhz/gradientBurn2', 'sadfox'));
		gradient.alpha = 0;

		blackvg = new FlxSprite().loadGraphic(Paths.image('black-vignette', 'sadfox'));
		blackvg.alpha = 0;
		...
		switch (curStage)
		{
			case 'void':
				//
			case 'pixel':
				var overlay:FlxSprite = new FlxSprite(0, 0).loadGraphic(Paths.image('overlay', 'sadfox'));
			...
			case 'hatch':
				...
				var ground:BGSprite = new BGSprite('stages/hatch/stand', -600, -200, 1, 1);
				add(ground);

				var bg:BGSprite = new BGSprite('stages/hatch/starpole', -800, -200, 0.4, 0.4);
				add(bg);

			case 'angel':
				var bg:BGSprite = new BGSprite('stages/angel/sky', -600, -200, 0.4, 0.4);
				add(bg);

				var leaves:BGSprite = new BGSprite('stages/angel/leaves', -600, -200, 0.6, 0.6);
				add(leaves);
			...
			case 'ghz': // Tails EXE
				var bg:BGSprite = new BGSprite('stages/ghz/skyHappy', -550, -300, 0.3, 0.3);
				...
				if (!ClientPrefs.lowQuality)
				{
					fore = new FlxSprite(-300, -100).loadGraphic(Paths.image('stages/ghz/foreHappy', 'sadfox'));
					fore.setGraphicSize(Std.int(fore.width * 2));
					fore.updateHitbox();
					fore.antialiasing = ClientPrefs.globalAntialiasing;
					fore.scrollFactor.set(1.4, 1.4);
					fore.active = false;
				}

				if (SONG.song.toLowerCase() == 'chasing')
				{
					bg2 = new BGSprite('stages/burningGhz/skyBurn', -550, -300, 0.3, 0.3);
					...
					fore2.updateHitbox();
					fore2.antialiasing = ClientPrefs.globalAntialiasing;
					fore2.scrollFactor.set(1.4, 1.4);
					fore2.visible = false;
					fore2.active = false;
					...
					tailsPxl.flipX = true;
					tailsPxl.animation.addByPrefix('idle', 'idle', 24, true);
					tailsPxl.animation.addByPrefix('braking', 'stop0', 24, false);
					tailsPxl.animation.addByPrefix('loopBraking', 'stopLoop', 24, true);
					tailsPxl.animation.addByPrefix('run', 'run', 24, true);
					tailsPxl.animation.addByPrefix('surprised', 'surprised', 24, true);
					...
					leTail.frames = Paths.getSparrowAtlas('cutscenes/chasing/tailsPixel', 'sadfox');
					leTail.antialiasing = false;
					leTail.flipX = true;
					leTail.animation.addByPrefix('brakTail', 'tailStop', 24, true);
					...
					sonicPxl.animation.addByPrefix('hiTails', 'heyBuddy', 24, false);
					...
					brakParticle.animation.addByPrefix('stopMan', 'brak', 24, true);
			...
			case 'burningGhz':
				...
				var bg:BGSprite = new BGSprite('stages/burningGhz/skyBurn', -550, -300, 0.3, 0.3);
				bg.setGraphicSize(Std.int(bg.width * 1.8));
				bg.updateHitbox();
				bg.antialiasing = ClientPrefs.globalAntialiasing;
				add(bg);
		...
		switch (curStage)
		{
			case 'ghz':
				...
				if (SONG.song.toLowerCase() == 'chasing')
				{
					...
					staticazzo1.cameras = [camHUD];
					staticazzo1.alpha = 0;
					staticazzo1.visible = false;
					add(staticazzo1);

					staticazzo2 = new FlxSprite(0, 0);
					staticazzo2.frames = Paths.getSparrowAtlas('delfino/sheeto2', 'sadfox');
					staticazzo2.color = FlxColor.RED;
					staticazzo2.setGraphicSize(FlxG.width, FlxG.height);
					staticazzo2.updateHitbox();
					staticazzo2.screenCenter();
					...
					vg = new FlxSprite().loadGraphic(Paths.image('RedVG', 'sadfox'));
					vg.alpha = 1;
					vg.cameras = [camHUD];
					vg.visible = false;
					add(vg);
		...
		if (SONG.song.toLowerCase() == 'sidekick')
		{
			egg = new Character(0, 0, "tailsH2");
			egg.alpha = 0;
			eggGroup.add(egg);
		}
		...
		var iconOffset = 56; // what?
		iconP1 = new HealthIcon(boyfriend.healthIcon, true);
		iconP1.y = healthBar.y - 75;
		iconP1.x = healthBar.x
			+ (healthBar.width * (FlxMath.remapToRange(0, 0, 100, 100, 0) * 0.01))
		...
		iconP2.x = healthBar.x
			+ (healthBar.width * (FlxMath.remapToRange(100, 0, 100, 100, 0) * 0.01))
	...
	public function addCharacterToList(newCharacter:String, type:Int)
	{
		switch (type)
		{
			...
			case 3:
				if (egg != null && !eggMap.exists(newCharacter))
				{
					var newEgg:Character = new Character(0, 0, newCharacter);
					newEgg.scrollFactor.set(0.95, 0.95);
					eggMap.set(newCharacter, newEgg);
					eggGroup.add(newEgg);
					startCharacterPos(newEgg);
					newEgg.alpha = 0.00001;
					startCharacterLua(newEgg.curCharacter);
				}
		}
	}
	...
	function sonecIntro():Void
	{
		inCutscene = true;

		var black:FlxSprite = new FlxSprite(0, 0).makeGraphic(FlxG.width * 2, FlxG.height * 2, FlxColor.BLACK);
		black.cameras = [camIntro];
		add(black);

		var blulol:FlxSprite = new FlxSprite(0, 0).makeGraphic(FlxG.width, FlxG.height, 0xFF1c46cc);
		blulol.y = -2000;
		blulol.updateHitbox();
		blulol.cameras = [camIntro];
		add(blulol);

		var giallol:FlxSprite = new FlxSprite(0, 0).makeGraphic(FlxG.width, FlxG.height, 0xFFecdd00);
		giallol.y = 2000;
		giallol.updateHitbox();
		giallol.cameras = [camIntro];
		add(giallol);

		var redshit:FlxSprite = new FlxSprite(0, 0).loadGraphic(Paths.image('redthing', 'sadfox'));
		redshit.setGraphicSize(FlxG.width, FlxG.height);
		redshit.x = -2000;
		redshit.updateHitbox();
		redshit.cameras = [camIntro];
		redshit.antialiasing = false;
		add(redshit);

		var sonecazzo:FlxText = new FlxText(0, FlxG.height * 0.2, 0, curSong, 80);
		sonecazzo.setFormat(Paths.font("siffredi.otf"), 80, FlxColor.WHITE, RIGHT);
		sonecazzo.x = 2000;
		sonecazzo.updateHitbox();
		sonecazzo.cameras = [camIntro];

		var bacazzo:FlxText = new FlxText(sonecazzo.x + 10, sonecazzo.y + 5, 0, sonecazzo.text, 80);
		bacazzo.setFormat(Paths.font("siffredi.otf"), 80, FlxColor.BLACK, RIGHT);
		bacazzo.x = 2000;
		bacazzo.updateHitbox();
		bacazzo.cameras = [camIntro];
		add(bacazzo);
		add(sonecazzo);
	...
	public function startCountdown():Void
	{
		...
		if (ret != FunkinLua.Function_Stop)
		{
			...
			startTimer = new FlxTimer().start(Conductor.crochet / 1000 / playbackRate, function(tmr:FlxTimer)
			{
				...
				if (egg != null
					&& tmr.loopsLeft % egg.danceEveryNumBeats == 0
					&& egg.animation.curAnim != null
					&& !egg.animation.curAnim.name.startsWith('sing')
					&& !egg.stunned)
				{
					egg.dance();
				}
				...
				{
					case 0:
						countdownPrepare = new FlxSprite().loadGraphic(Paths.image(introAlts[3]));
						...
						FlxTween.tween(countdownPrepare, {/*y: countdownReady.y + 100,*/ alpha: 0}, Conductor.crochet / 1000, {
	...
	private function generateStaticArrows(player:Int):Void
	{
		for (i in 0...4)
		{
			...
			if (player < 1)
			{
				var hideNotes = !ClientPrefs.opponentStrums;
				for (song in ['hatch', 'starshine', 'levitating'])
				{
					if (curSong.toLowerCase() == song)
						hideNotes = true;
				}
			...
			if (!isStoryMode && !skipArrowStartTween)
			{
				...
				ClientPrefs.downScroll ? babyArrow.y += 100 : babyArrow.y -= 100;

				FlxTween.tween(babyArrow, {y: ClientPrefs.downScroll ? babyArrow.y - 100 : babyArrow.y + 100, alpha: targetAlpha}, 1,
					{ease: FlxEase.circOut, startDelay: 0.6 + (0.15 * i)});
			}
			else
			{
	...
	override public function update(elapsed:Float)
	{
		...
		if (followTailSprite && tailsPxl.visible)
			triggerEventNote('Camera Follow Pos', Std.string(tailsPxl.x), Std.string(tailsPxl.y - 30));
		...
		if (SONG.notes[curSection].mustHitSection != lastMustHit)
		{
			...
			if (!lastMustHit && curCombo > 12 && (curBeat % 4 == 0 || curBeat % 6 == 0))
			{
				trace("combo!");
				curCombo = 0;
		...
		if (!inCutscene)
		{
			...
			camFollowPos.setPosition(CoolUtil.coolLerp(camFollowPos.x, camFollow.x + camDisplaceX, lerpVal),
				CoolUtil.coolLerp(camFollowPos.y, camFollow.y + camDisplaceY, lerpVal));
			if (!startingSong
				&& !endingSong
				&& boyfriend.animation.curAnim != null
		...
		if (FlxG.keys.justPressed.SEVEN
			&& (ClientPrefs.areYouTeles || SongUnlock.getStoryStatus() <= 11)
			&& !endingSong
			&& !inCutscene)
		{
			openChartEditor();
	...
	function doDeathCheck(?skipHealthCheck:Bool = false)
	{
		if (((skipHealthCheck && instakillOnMiss) || health <= 0) && !practiceMode && !isDead)
		{
			...
			if (ret != FunkinLua.Function_Stop)
			{
				...
				openSubState(new ExeSubstate(boyfriend.getScreenPosition().x - boyfriend.positionArray[0],
		...
		switch (eventName)
		{
			...
			case 'Change Character':
				...
				switch (charType)
				{
					...
					case 3:
						if (egg != null && egg.curCharacter != value2)
						{
							if (!eggMap.exists(value2))
							{
								addCharacterToList(value2, charType);
							}

							var lastAlpha:Float = egg.alpha;
							egg.alpha = 0.00001;
							egg = eggMap.get(value2);
							egg.alpha = lastAlpha;
						}
				}
				reloadHealthBarColors();
	...
	{
		if (isDad || SONG.song.toLowerCase() == 'levitating' || SONG.song.toLowerCase() == 'starshine')
		{
	...
	public function endSong():Void
	{
		...
		if (ret != FunkinLua.Function_Stop && !transitioning)
		{
			...
			else
			{
				...
				if (curSong.toLowerCase() == 'SCRAPPED')
					MusicBeatState.switchState(new ComingSoonScreen());
				else
				{
	...
	private function springNoteThing():Void
	{
		if (InOutThingy == 'In')
		{
			for (i in 0...opponentStrums.length)
			{
				FlxTween.tween(opponentStrums.members[0], {x: opponentStrums.members[0].x + 80}, Conductor.crochet / 200, {ease: FlxEase.quadOut});
				FlxTween.tween(opponentStrums.members[1], {x: opponentStrums.members[1].x + 40}, Conductor.crochet / 200, {ease: FlxEase.quadOut});
				FlxTween.tween(opponentStrums.members[2], {x: opponentStrums.members[2].x - 40}, Conductor.crochet / 200, {ease: FlxEase.quadOut});
				FlxTween.tween(opponentStrums.members[3], {x: opponentStrums.members[3].x - 80}, Conductor.crochet / 200, {ease: FlxEase.quadOut});
			}

			for (i in 0...playerStrums.length)
			{
				FlxTween.tween(playerStrums.members[0], {x: playerStrums.members[0].x + 80}, Conductor.crochet / 200, {ease: FlxEase.quadOut});
				FlxTween.tween(playerStrums.members[1], {x: playerStrums.members[1].x + 40}, Conductor.crochet / 200, {ease: FlxEase.quadOut});
				FlxTween.tween(playerStrums.members[2], {x: playerStrums.members[2].x - 40}, Conductor.crochet / 200, {ease: FlxEase.quadOut});
				FlxTween.tween(playerStrums.members[3], {x: playerStrums.members[3].x - 80}, Conductor.crochet / 200, {ease: FlxEase.quadOut});
			}
			InOutThingy = 'Out';
		}
		else if (InOutThingy == 'Out')
		{
			for (i in 0...opponentStrums.length)
			{
				FlxTween.tween(opponentStrums.members[0], {x: opponentStrums.members[0].x - 80}, Conductor.crochet / 200, {ease: FlxEase.quadOut});
		...
		else if (InOutThingy == 'Special')
		{
			for (i in 0...opponentStrums.length)
			{
				FlxTween.tween(opponentStrums.members[0], {x: opponentStrums.members[0].x + 40}, Conductor.crochet / 200, {ease: FlxEase.quadOut});
				FlxTween.tween(opponentStrums.members[1], {x: opponentStrums.members[1].x + 20}, Conductor.crochet / 200, {ease: FlxEase.quadOut});
				FlxTween.tween(opponentStrums.members[2], {x: opponentStrums.members[2].x - 20}, Conductor.crochet / 200, {ease: FlxEase.quadOut});
				FlxTween.tween(opponentStrums.members[3], {x: opponentStrums.members[3].x - 40}, Conductor.crochet / 200, {ease: FlxEase.quadOut});
			}

			for (i in 0...playerStrums.length)
			{
				FlxTween.tween(playerStrums.members[0], {x: playerStrums.members[0].x + 40}, Conductor.crochet / 200, {ease: FlxEase.quadOut});
				FlxTween.tween(playerStrums.members[1], {x: playerStrums.members[1].x + 20}, Conductor.crochet / 200, {ease: FlxEase.quadOut});
				FlxTween.tween(playerStrums.members[2], {x: playerStrums.members[2].x - 20}, Conductor.crochet / 200, {ease: FlxEase.quadOut});
				FlxTween.tween(playerStrums.members[3], {x: playerStrums.members[3].x - 40}, Conductor.crochet / 200, {ease: FlxEase.quadOut});
			}
			InOutThingy = 'Out';
		}
		else if (InOutThingy == 'Ending')
		{
			for (i in 0...opponentStrums.length)
			{
				FlxTween.tween(opponentStrums.members[0], {x: opponentStrums.members[0].x - 40}, Conductor.crochet / 200, {ease: FlxEase.quadOut});
	...
	override function stepHit()
	{
		...
		switch (SONG.song.toLowerCase())
		{
			...
			case 'starshine':
				...
				{
					case 1:
						FlxTween.tween(blackSkit, {alpha: 0}, 20, {ease: FlxEase.quadInOut});
					case 128:
						dad.alpha = 1;
					case 1536:
						FlxTween.tween(dad, {alpha: 0}, 3, {ease: FlxEase.quadInOut});
					case 1556:
						FlxTween.tween(blackSkit, {alpha: 1}, 3, {ease: FlxEase.quadInOut});
				}
			case 'nightmare':
				switch (curStep)
				{
			...
			case 'chasing':
				...
				switch (curStep)
				{
					...
					case 408:
						...
						switchToHell(true);
						blakkorekt = new FlxSprite(-FlxG.width * 2,
							-FlxG.height * 2).makeGraphic(Std.int(FlxG.width * 5), Std.int(FlxG.height * 5), FlxColor.BLACK);
						add(blakkorekt);
						camZooming = false;
						add(animSky);
						add(animStuff);
					...
					case 420:
						...
						brakParticle.animation.play('stopMan');
						leTail.x = leTail.x + 40;
					...
					case 592:
						...
						camHUD.shake(0.008, 0.1);
						staticazzo1.animation.play('cannella');
						staticazzo1.visible = true;
						FlxTween.tween(staticazzo1, {alpha: 1}, Conductor.crochet / 200, {ease: FlxEase.quadIn});
					case 608:
						triggerEventNote('Camera Follow Pos', '', '');
						staticazzo1.visible = false;
						defaultCamZoom = 0.55;
					...
					case 1152 | 1184 | 1216:
						defaultCamZoom = 0.55;
					case 736 | 992:
						camHUD.flash(FlxColor.WHITE, Conductor.crochet / 1000);
						defaultCamZoom = 0.55;
					case 864:
						camHUD.flash(FlxColor.RED, Conductor.crochet / 1000);
						defaultCamZoom = 0.55;
					case 1120:
						...
						defaultCamZoom = 0.55;
						vg.visible = true;
						FlxTween.tween(vg, {alpha: 0}, Conductor.crochet / 200, {type: PINGPONG});
					case 1248:
						camHUD.flash(FlxColor.RED, Conductor.crochet / 1000);
						springNoteThing();
						defaultCamZoom = 0.55;
					case 1264 | 1296 | 1328 | 1360:
						springNoteThing();
						defaultCamZoom += 0.3;
					case 1280 | 1312 | 1344 | 1392 | 1424 | 1456 | 1488 | 1520 | 1552 | 1584:
						springNoteThing();
						defaultCamZoom = 0.55;
					case 1376:
						springNoteThing();
						...
						// timer pazzo
						timeTxt.visible = false;
						cloneTimeTxt.visible = true;
					// FlxTween.cancelTweensOf(vg);
					// FlxTween.tween(vg, {alpha: 0}, Conductor.crochet / 1000, {type: PINGPONG});
					case 1408 | 1440 | 1472 | 1504 | 1536 | 1568:
						springNoteThing();
						defaultCamZoom += 0.3;
						camHUD.flash(FlxColor.RED, Conductor.crochet / 1000);
					case 1600:
						springNoteThing();
						camHUD.flash(FlxColor.RED, Conductor.crochet / 1000);
						triggerEventNote('Camera Follow Pos', Std.string(boyfriend.getMidpoint().x - dad.getMidpoint().x), Std.string(boyfriend.getMidpoint().y));
					case 1616:
						InOutThingy = 'Special';
						springNoteThing();
						triggerEventNote('Camera Follow Pos', '', '');
					case 1632:
						camHUD.flash(FlxColor.WHITE, Conductor.crochet / 1000);
						triggerEventNote('Camera Follow Pos', Std.string(boyfriend.getMidpoint().x - dad.getMidpoint().x + 500), '900');
						FlxTween.cancelTweensOf(vg);
						vg.destroy();
						timeTxt.visible = true;
						cloneTimeTxt.visible = false;
					case 1656:
						FlxG.camera.fade(FlxColor.BLACK, Conductor.crochet / 1000, false);
						FlxTween.tween(camHUD, {alpha: 0}, Conductor.crochet / 200, {ease: FlxEase.expoOut});
					case 1700:
						FlxG.sound.play(Paths.sound('cazzoRidi', 'sadfox'));
				}

				if (curStep >= 112 && curStep < 128 || curStep >= 1632 && curStep < 1656)
				{
					defaultCamZoom += 0.01;
				}
				if (curStep >= 480 && curStep < 592)
				{
					defaultCamZoom -= 0.008;
				}
			case 'soic':
				switch (curStep)
				{
					case 80:
						...
						dad.specialAnim = true;
					case 115:
						FlxTween.tween(blackSkit, {alpha: 0}, 0.5, {ease: FlxEase.linear});
					...
					case 512:
						...
						FlxG.camera.zoom += 0.015;
						camHUD.zoom += 0.03;
					case 528 | 538 | 556 | 566 | 580 | 596 | 736 | 744 | 752 | 756 | 1152 | 1168 | 1169 | 1178 | 1196 | 1206 | 1220 | 1236 | 1376 | 1384 | 1392 |
						1396:
						FlxG.camera.zoom += 0.025;
						camHUD.zoom += 0.035;
					case 608 | 616 | 624 | 628 | 632 | 634 | 1248 | 1256 | 1264 | 1268 | 1272 | 1274:
				...
				if (curStep >= 115 && curStep < 127 || curStep >= 224 && curStep < 240 || curStep >= 352 && curStep < 368 || curStep >= 992 && curStep < 1008
					|| curStep >= 1504 && curStep < 1520 || curStep >= 1632 && curStep < 1648)
				{
					defaultCamZoom += 0.01;
				}
				...
				if (curStep >= 192 && curStep < 200 || curStep >= 204 && curStep < 212 || curStep >= 216 && curStep < 220 || curStep >= 352 && curStep < 368
					|| curStep >= 448 && curStep < 456 || curStep >= 460 && curStep < 468 || curStep >= 472 && curStep < 476 || curStep >= 480 && curStep < 496
					|| curStep >= 760 && curStep < 764 || curStep >= 960 && curStep < 968 || curStep >= 972 && curStep < 980 || curStep >= 984 && curStep < 988
					|| curStep >= 1400 && curStep < 1404 || curStep >= 1472 && curStep < 1480 || curStep >= 1484 && curStep < 1492 || curStep >= 1496
					&& curStep < 1500 || curStep >= 1600 && curStep < 1608 || curStep >= 1612 && curStep < 1620 || curStep >= 1624 && curStep < 1628)
				{
	...
	override function beatHit()
	{
		...
		if (egg != null
			&& curBeat % egg.danceEveryNumBeats == 0
			&& egg.animation.curAnim != null
			&& !egg.animation.curAnim.name.startsWith('sing')
			&& !egg.stunned)
		{
		...
		if (curSong.toLowerCase() == 'soic' && curBeat >= 32 && curBeat < 60 && FlxG.camera.zoom < 1.35 && ClientPrefs.camZooms)
		{
			FlxG.camera.zoom += 0.05;
			camHUD.zoom += 0.06;
		}
		if (curSong.toLowerCase() == 'soic' && curBeat >= 60 && curBeat < 64 && FlxG.camera.zoom < 1.35 && ClientPrefs.camZooms)
		{
			FlxG.camera.zoom += 0.015;
			camHUD.zoom += 0.03;
		}
		if (curSong.toLowerCase() == 'soic' && curBeat >= 64 && curBeat < 92 && FlxG.camera.zoom < 1.35 && ClientPrefs.camZooms)
		{
			FlxG.camera.zoom += 0.05;
			camHUD.zoom += 0.06;
		}
		if (curSong.toLowerCase() == 'soic' && curBeat >= 96 && curBeat < 104 && FlxG.camera.zoom < 1.35 && ClientPrefs.camZooms)
		{
			FlxG.camera.zoom += 0.015;
			camHUD.zoom += 0.03;
		}
		if (curSong.toLowerCase() == 'soic' && curBeat >= 104 && curBeat < 128 && FlxG.camera.zoom < 1.35 && ClientPrefs.camZooms)
		{
			FlxG.camera.zoom += 0.05;
			camHUD.zoom += 0.06;
		}
		if (curSong.toLowerCase() == 'soic' && curBeat >= 192 && curBeat < 224 && FlxG.camera.zoom < 1.35 && ClientPrefs.camZooms)
		{
			FlxG.camera.zoom += 0.015;
			camHUD.zoom += 0.03;
		}
		if (curSong.toLowerCase() == 'soic' && curBeat >= 224 && curBeat < 288 && FlxG.camera.zoom < 1.35 && ClientPrefs.camZooms)
		{
			FlxG.camera.zoom += 0.05;
			camHUD.zoom += 0.06;
		}
		if (curSong.toLowerCase() == 'soic' && curBeat >= 352 && curBeat < 360 && FlxG.camera.zoom < 1.35 && ClientPrefs.camZooms)
		{
			FlxG.camera.zoom += 0.015;
			camHUD.zoom += 0.03;
		}
		if (curSong.toLowerCase() == 'soic' && curBeat >= 360 && curBeat < 380 && FlxG.camera.zoom < 1.35 && ClientPrefs.camZooms)
		{
			FlxG.camera.zoom += 0.05;
			camHUD.zoom += 0.06;
		}
		if (curSong.toLowerCase() == 'soic' && curBeat >= 380 && curBeat < 384 && FlxG.camera.zoom < 1.35 && ClientPrefs.camZooms)
		{
			FlxG.camera.zoom += 0.015;
			camHUD.zoom += 0.03;
		}
		if (curSong.toLowerCase() == 'soic' && curBeat >= 384 && curBeat < 416 && FlxG.camera.zoom < 1.35 && ClientPrefs.camZooms)
		{
			FlxG.camera.zoom += 0.05;
			camHUD.zoom += 0.06;
		}

		if (curSong.toLowerCase() == 'chasing' && curBeat >= 152 && curBeat < 216 && FlxG.camera.zoom < 1.35 && ClientPrefs.camZooms)
		{
			FlxG.camera.zoom += 0.015;
			camHUD.zoom += 0.03;
		}
		if (curSong.toLowerCase() == 'chasing' && curBeat >= 216 && curBeat < 248 && FlxG.camera.zoom < 1.35 && ClientPrefs.camZooms)
		{
			FlxG.camera.zoom += 0.03;
			camHUD.zoom += 0.04;
		}
		if (curSong.toLowerCase() == 'chasing' && curBeat >= 248 && curBeat < 276 && FlxG.camera.zoom < 1.35 && ClientPrefs.camZooms)
		{
			FlxG.camera.zoom += 0.015;
			camHUD.zoom += 0.03;
		}
		if (curSong.toLowerCase() == 'chasing' && curBeat >= 276 && curBeat < 280 && FlxG.camera.zoom < 1.35 && ClientPrefs.camZooms)
		{
			FlxG.camera.zoom += 0.03;
			camHUD.zoom += 0.04;
		}
		if (curSong.toLowerCase() == 'chasing' && curBeat >= 312 && curBeat < 344 && FlxG.camera.zoom < 1.35 && ClientPrefs.camZooms)
		{
			FlxG.camera.zoom += 0.015;
			camHUD.zoom += 0.03;
		}
		if (curSong.toLowerCase() == 'chasing' && curBeat >= 344 && curBeat < 408 && FlxG.camera.zoom < 1.35 && ClientPrefs.camZooms)
		{
			FlxG.camera.zoom += 0.05;
			camHUD.zoom += 0.05;
		}
